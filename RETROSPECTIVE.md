...
| `vllm/utils/math_utils.py:15` (`next_power_of_2`) | math helpers | non-buggy + buggy pair on the verbatim upstream form (4 VCCs each); the loop-reimplementation workaround for [esbmc/esbmc#4756](https://github.com/esbmc/esbmc/issues/4756) was retired once [esbmc/esbmc#4757](https://github.com/esbmc/esbmc/pull/4757) landed |
| `vllm/utils/math_utils.py:30` (`largest_power_of_2_divisor`) | math helpers | non-buggy + buggy pair on the verbatim upstream form (6 VCCs each); same fix |
...
| [esbmc/esbmc#4756](https://github.com/esbmc/esbmc/issues/4756) | **RESOLVED** ([#4757](https://github.com/esbmc/esbmc/pull/4757), workaround now retired) | "`int.bit_length()` OM unwinds indefinitely on symbolic input despite tight `__ESBMC_assume` bound" | Loop-reimplementation workaround in `next_power_of_2` / `largest_power_of_2_divisor` retired; both targets now verify the verbatim upstream bit-trick forms. |
...
| Bit-trick targets (`next_power_of_2`, `largest_power_of_2_divisor`) | Loop reimplementations using only `+`, `-`, `*`, `//`, `%`, comparison, with `--unwind 32` in the manifest | [esbmc/esbmc#4757](https://github.com/esbmc/esbmc/pull/4757) (fixes [#4756](https://github.com/esbmc/esbmc/issues/4756)) | **Retired.** Both targets now verify the verbatim upstream form (`1 << (n - 1).bit_length()` and `n & (-n)`). `--unwind 32` dropped from the manifest entries; ESBMC's `bit_length` OM now self-bounds at length 512. |
...
3. **~~Bitwise / shift operational models on bigint Python need attention.~~** Filed as [esbmc/esbmc#4756](https://github.com/esbmc/esbmc/issues/4756) and **resolved** by [esbmc/esbmc#4757](https://github.com/esbmc/esbmc/pull/4757). The `bit_length` OM now self-bounds at length 512, covering both narrow 64-bit `IntWide` and `--ir` bignum (512-bit) call sites; both Tier-1 targets that needed loop reimplementations now verify the verbatim upstream forms.
