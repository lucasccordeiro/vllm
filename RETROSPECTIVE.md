...
- **One further live finding — programmatic, not CLI-reachable** (AUDIT.md Finding #8): a negative `SchedulerConfig.max_num_scheduled_tokens` reaches `schedule()`'s token budget and trips a bare `assert token_budget >= 0`. The defect is a *gated* guard — the only `<= 0` check is inside `_set_max_num_scheduled_tokens`, which runs only under speculative decoding — so the same value is a clean `ValueError` with spec and a cryptic `AssertionError` without. Harnessed (`max_num_scheduled_tokens_negative.py`, FAILED, 1 VCC). Distinct from the seven above because it is not triggerable via `vllm serve` flags; filed (as an explicitly lower-severity, integrator-facing report) as [vllm-project/vllm#44123](https://github.com/vllm-project/vllm/issues/44123).
