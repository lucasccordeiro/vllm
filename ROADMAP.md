...
| `--hash-block-size -1` propagation | `vllm/v1/core/kv_cache_coordinator.py:429-431`, `vllm/v1/kv_offload/base.py:363` | none | Adjacent failure mode to #43521. Unverified data-corruption hypothesis: `bs % -1 == 0` for non-negative `bs`, so the negative value silently passes the existing validator and propagates downstream. Needs its own harness + sandbox reproducer before filing. |
