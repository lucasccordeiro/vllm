...
| `--max-model-len 0` | `vllm/engine/arg_utils.py:802` → `vllm/v1/core/sched/scheduler.py:397` | none | ✅ **Shipped and confirmed live.** Harness `max_model_len_zero_cli_path.py`; ESBMC counterexample at `max_model_len=0, num_computed_tokens=0, num_new_tokens=1 → -1`; empirical chain (`_get_and_verify_max_len(0) = 0`; `min(1, -1) = -1`; `cdiv(-1, 16) = 0`) confirms silent propagation. See [`AUDIT.md`](./AUDIT.md) Finding #3. Upstream issue to file. |
...
|---|---|---|---|
| End of Tier 1 + both Tier 2 audits + four Tier 2 harnesses (current) | 16 entries | ~74 s | **4** (filed: #43496 + #43521; queued: `--hash-block-size -k` propagation as comment for #43521; new: `--max-model-len 0` silent negative-num_new_tokens propagation, upstream issue draft pending) |
