...
| `Scheduler.schedule()` token-budget loop | `vllm/v1/core/sched/scheduler.py:656,672` | symbolic scheduler/request scalars | 🚧 **First slice shipped, Phase 1 + Phase 2 SUCCESSFUL** (`scheduler_token_budget.py`, 3 / 13 VCCs). Verifies the **inductive step** of the running-loop body: one iteration preserves the flagship `token_budget >= 0` invariant (guaranteed by the `min(num_new_tokens, token_budget)` clamp) and yields `num_new_tokens >= 0`, and the preemption-restore branch (`token_budget += num_scheduled_tokens.pop(...)`) preserves it. The `num_computed_tokens <= max_model_len - 1` precondition is exactly check_stop's length-cap invariant (row 3) — composition point. Buggy counterpart drops the budget clamp → `token_budget < 0`, FAILS. **Remaining slices:** mechanise the full K-request loop (cross-iteration `token_budget`/`num_scheduled_tokens` accumulation), the actual running-list mutation on preemption, and multiple preemptions. Multi-week. |
...
| Tier 4 rows 1–3 + `schedule()` flagship first slice shipped | 36 entries | ~4 min 40 s | open |
| End of Tier 4 | remaining `schedule()` slices (full loop, running-list mutation, multi-preemption) | ~5–8 min (CI-relevant) | open |
