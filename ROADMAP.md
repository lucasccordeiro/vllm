...
| `FreeKVCacheBlockQueue.popleft_n(n)` | `vllm/v1/core/kv_cache_utils.py:253` | doubly-linked `KVCacheBlock` at concrete K = 4 | ✅ **Shipped, Phase 1 + Phase 2 SUCCESSFUL at K = 4** (`free_kv_cache_block_queue_popleft_n.py`, 2321 VCCs, ~33 s/phase). Buggy counterpart (drops the `prev[curr] = HEAD` reconnect) FAILS as expected. Doubly-linked structure modelled via parallel int arrays + integer sentinels (NIL = -1, HEAD = 4, TAIL = 5) to sidestep ESBMC-Python's `Optional`/None and nested-attribute gaps. Two ESBMC-Python frontend pitfalls hit while building: (i) module-level `HEAD = K` (named constant referencing another) triggers `nlohmann::json::operator[]` crash at GOTO generation; (ii) `for i in range(K): if cond: continue` expands to an internal secondary loop with very high unwind ID (loop 149 in trial runs). Both worked around in the harness; the first will be filed as an upstream ESBMC issue. |
...
8. **Tier 3, row 4** (`BlockPool.get_new_blocks`) — ✅ shipped. First ref-counting layer atop the verified free-list pop; Phase 1 + 2 SUCCESSFUL at K = 4 (`block_pool_get_new_blocks.py`). **Row 5** (`KVCacheManager.allocate_slots`) remains: it is the coordinator that composes everything, depends on a five-method `coordinator` stub plus a `Request` stub, and its verifiable surface is the token-accounting arithmetic (`num_tokens_main_model = total_computed_tokens + num_new_tokens`, the `min(…, max_model_len)` saturations) and the `num_blocks_to_allocate > get_num_free_blocks()` admission guard. Different layer from the free-list queue; tracked as the next discrete piece. ~half a day.
