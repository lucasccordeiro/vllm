...
|---|---|---|---|
| End of Tier 1 + both Tier 2 audits + five Tier 2 harnesses (current) | 17 entries | ~83 s | **5 filed**: #43496 + #43521 + #43532 + #43842 + the unfiled `--hash-block-size -k` propagation. Four resolved by upstream [#43794](https://github.com/vllm-project/vllm/pull/43794) (the first three explicitly; `--hash-block-size -k` incidentally via the same `gt=0`). One still open: [#43842](https://github.com/vllm-project/vllm/issues/43842) (`--num-gpu-blocks-override 0` bare `AssertionError`). |
