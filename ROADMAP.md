...
9. **Tier 4** in priority order: ~~`_has_repeating_pattern`~~ — ✅ shipped (negative-index safety at K = 8). ~~`check_sequence_repetition`~~ — ✅ shipped (composes with row 1 into end-to-end detector safety). ~~`check_stop`~~ — ✅ shipped (length-cap lifecycle invariant + index safety). Next: the flagship `schedule()` token-budget loop. Multi-week.
...
| Tier 4 rows 1–3 (`_has_repeating_pattern`, `check_sequence_repetition`, `check_stop`) shipped | 34 entries | ~4 min 35 s | open |
| End of Tier 4 | +2 entries → ~36 (flagship `schedule()` token-budget loop) | ~5–8 min (CI-relevant) | open |
