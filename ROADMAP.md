...
9. **Tier 4** in priority order: ~~`_has_repeating_pattern`~~ — ✅ shipped (negative-index safety at K = 8). ~~`check_sequence_repetition` (the guard wrapper)~~ — ✅ shipped (composes with row 1 into end-to-end detector safety). Next: `check_stop`, then the flagship `schedule()` token-budget loop. The last is multi-week.
...
| Tier 4 rows 1–2 (`_has_repeating_pattern`, `check_sequence_repetition`) shipped | 32 entries | ~4 min 30 s | open |
| End of Tier 4 | +2 entries → ~34 (`check_stop`, `schedule()`) | ~5–8 min (CI-relevant) | open |
