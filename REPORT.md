...
| 29 | `kv_cache_memory_bytes` negative budget (live finding, **CLI-wired**) | `vllm/v1/worker/gpu_worker.py:370,388` → `vllm/v1/core/block_pool.py:157` (ninth live finding; **corrects an `AUDIT.md` mis-classification** — the field is CLI-wired (`--kv-cache-memory-bytes`, `arg_utils.py:1122`), not engine-state. `CacheConfig.kv_cache_memory_bytes` (`int\|None`, no `gt=`) reaches `gpu_worker.py:370`'s truthiness walrus `if kv_cache_memory_bytes := …`, which treats a negative as "explicitly set" and returns it verbatim; `get_num_blocks`'s `max(num_blocks, 0)` clamp drives it to `0`, tripping `BlockPool`'s bare `assert num_gpu_blocks > 0`. CLI-reachable on the default GPU path; truthiness class of #44123 combined with the #43842 crash site. Not yet filed) |
...
| `kv_cache_memory_bytes_negative_cli_path`        | **FAILED (live bug witness, CLI-wired `--kv-cache-memory-bytes -1`; truthiness walrus at `gpu_worker.py:370` returns the negative verbatim, `get_num_blocks`'s `max(.,0)` clamp → `num_blocks=0`, bare `assert num_gpu_blocks > 0` at `block_pool.py:157`; corrects an AUDIT mis-classification, #44123 truthiness class + #43842 crash site, not yet filed)** | skipped | 4    |
