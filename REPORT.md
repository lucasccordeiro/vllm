...
| 12 | `--max-logprobs <negative>` CLI path | `vllm/engine/arg_utils.py:525` → `vllm/sampling_params.py:680` (silent acceptance of every negative except the `-1` sentinel) |
| 13 | `--long-prefill-token-threshold <negative>` CLI path | `vllm/engine/arg_utils.py:1386` → `vllm/v1/core/sched/scheduler.py:390` (`0 < threshold < num_new_tokens` guard silently no-ops on negatives) |
...
| 22 | `Scheduler.schedule()` token-budget invariant (first slice) | `vllm/v1/core/sched/scheduler.py:348,366` (Tier-4 flagship; the running-loop inductive step + preemption-restore preserve `token_budget >= 0` and yield `num_new_tokens >= 0`, composing with #21's length-cap invariant) |
...
| 27 | `max_num_scheduled_tokens` negative budget (live finding, **programmatic — not CLI**) | `vllm/v1/core/sched/scheduler.py:348,829` (eighth live finding, from the programmatic-int-fields audit; `SchedulerConfig.max_num_scheduled_tokens` has no constraint and the only `<= 0` guard — `vllm/config/vllm.py:1566` — is gated behind `speculative_config is not None`. Without spec decoding a negative survives, the truthiness fallback at `scheduler.py:104` propagates it, and `schedule()` trips the bare `assert token_budget >= 0`. A *gated*-guard inconsistency, same bare-`AssertionError` class as #43842; filed as [vllm-project/vllm#44123](https://github.com/vllm-project/vllm/issues/44123), **fixed upstream** by [#44207](https://github.com/vllm-project/vllm/pull/44207) — merged `Field(default=None, ge=0)` + explicit `is not None` fallback) |
| 28 | `Scheduler.schedule()` encoder compute-budget invariant | `vllm/v1/core/sched/scheduler.py:355,478` (Tier-4 flagship; the **third** budget surface — `encoder_compute_budget`; unlike the token budget's `min(·)` clamp, deducts only behind the `can_allocate` guard `num_embeds <= budget` (`encoder_cache_manager.py:157`) and skips otherwise, with a preemption restore at `scheduler.py:478`. Proves `0 <= encoder_compute_budget <= B` and the accounting identity across guarded deducts + restores) |
| 29 | `kv_cache_memory_bytes` admission guard (**not a live bug**) | `vllm/v1/core/kv_cache_utils.py:697,709` (Finding #9, investigated and closed without a finding — Finding #7 class. The field is CLI-wired (`--kv-cache-memory-bytes`, `arg_utils.py:1122`) and unconstrained, and `gpu_worker.py:370`'s truthiness walrus returns a negative verbatim, but `_check_enough_kv_cache_memory` rejects a non-positive / sub-one-block budget with a clean `ValueError` before `get_num_blocks`' clamp or `block_pool.py:157`. Proves (SUCCESSFUL) the admission guard establishes `BlockPool`'s `num_gpu_blocks > 0` precondition. An earlier harness reported FAILED on an incomplete call chain that omitted this guard) |
...
| `num_gpu_blocks_override_zero_cli_path` | **FAILED (live bug witness, [vllm-project/vllm#43842](https://github.com/vllm-project/vllm/issues/43842), bare `AssertionError` at `block_pool.py:157`; **still open and unfixed upstream** as of 2026-08-27 — `main @ 4a6a3272` keeps the unconstrained field and the bare assert, now at `block_pool.py:170`)** | skipped         | 1    |
| `max_logprobs_negative_cli_path` | **FAILED (silent-config-acceptance witness, filed [vllm-project/vllm#43985](https://github.com/vllm-project/vllm/issues/43985), **fixed upstream** by [#44070](https://github.com/vllm-project/vllm/pull/44070) — `Field(ge=-1)`; field admitted any negative besides the `-1` sentinel, surfacing either a confusing "max allowed: -5" error or a pure no-op depending on whether requests opt into logprobs)** | skipped         | 1    |
| `long_prefill_token_threshold_negative_cli_path` | **FAILED (silent-config-acceptance witness, filed [vllm-project/vllm#43985](https://github.com/vllm-project/vllm/issues/43985), **fixed upstream** by [#44070](https://github.com/vllm-project/vllm/pull/44070) — `Field(ge=0)`; field admitted any negative, scheduler.py:390 guard `0 < threshold < num_new_tokens` silently no-oped, user-set cap had zero effect)** | skipped         | 1    |
...
| `scheduler_token_budget_preempt` (K = 4)         | SUCCESSFUL (expected)         | SUCCESSFUL (expected)        | 17   |
...
| `scheduler_encoder_budget` (K = 4)               | SUCCESSFUL (expected)         | SUCCESSFUL (expected)        | 17   |
| `scheduler_encoder_budget_buggy`                 | FAILED (expected; `can_allocate` budget guard dropped, unguarded `encoder_compute_budget -= num_encoder_embeds` drives the budget negative, P1 `>= 0` violated) | skipped | 17   |
...
| `max_num_scheduled_tokens_negative`              | **FAILED (live bug witness, programmatic — not CLI; `spec_config_present=0, max_num_scheduled_tokens=-1` → `token_budget=-1`, bare `assert token_budget >= 0` at `scheduler.py:829`; gated-guard inconsistency, filed as [#44123](https://github.com/vllm-project/vllm/issues/44123), **fixed upstream** by [#44207](https://github.com/vllm-project/vllm/pull/44207), same class as [#43842](https://github.com/vllm-project/vllm/issues/43842))** | skipped | 1    |
| `kv_cache_memory_bytes_admission_guard`          | SUCCESSFUL (not a live bug; proves `_check_enough_kv_cache_memory` at `kv_cache_utils.py:697,709` establishes `BlockPool`'s `num_gpu_blocks > 0` precondition — a non-positive / sub-one-block `--kv-cache-memory-bytes` is rejected with a clean `ValueError` before the bare assert. Finding #7 class; an earlier harness FAILED on an incomplete chain that omitted this guard) | SUCCESSFUL | 4 / 17 |
